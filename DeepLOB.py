...
class DeepLOB(nn.Module):
    def __init__(self, num_features: int, num_classes: int = 3):
        ...
        self.lstm = nn.LSTM(input_size=48, hidden_size=64, batch_first=True, bidirectional=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Output shape after concatenation: [batch, 48, time_steps:200, features:1]

        # LSTM Block
        # Reshape from [batch, 48, time_steps, features:1] to [batch, time_steps, features:48]
...
class FE_DeepLOB(nn.Module):
    def __init__(self, num_features: int, num_classes: int = 3):
        ...
        self.conv_time1 = nn.Conv2d(1, 8, kernel_size=(4, 1))
        ...
        self.conv_feat1 = nn.Conv2d(8, 8, kernel_size=(1, num_features))
        self.conv_time5 = nn.Conv2d(8, 8, kernel_size=(4, 1))
        self.conv_time6 = nn.Conv2d(8, 8, kernel_size=(4, 1))

        # Inception layer would be defined here (not implemented in this simple version)
        self.incp_Path1_1 = nn.Conv2d(8, 16, kernel_size=(1,1))
        self.incp_Path1_pad = nn.ZeroPad2d((0, 0, 2, 0))  # Pad only on the top for time dimension
        self.incp_Path1_2 = nn.Conv2d(16, 16, kernel_size=(3,1))

        self.incp_Path2_1 = nn.Conv2d(8, 16, kernel_size=(1,1))
        self.incp_Path2_pad = nn.ZeroPad2d((0, 0, 4, 0))  # Pad only on the top for time dimension
        self.incp_Path2_2 = nn.Conv2d(16, 16, kernel_size=(5,1))

        self.incp_Path3_pad = nn.ConstantPad2d((0, 0, 2, 0), float('-inf'))  # Pad only on the top for time dimension
        self.incp_Path3_1 = nn.MaxPool2d(kernel_size=(3,1), stride=(1,1))
        self.incp_Path3_2 = nn.Conv2d(8, 16, kernel_size=(1,1))

        # LSTM layer with 64 hidden units
        self.lstm = nn.LSTM(input_size=48, hidden_size=32, batch_first=True, bidirectional=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.causal_pad(x)  # Apply causal padding before the time convolution
        x = F.leaky_relu(self.conv_time6(x), negative_slope=0.01)
        # Output shape after conv_time6: [batch, 8, time_steps:200, features:1] 
...
class PnLFocalLoss(nn.Module):
    def __init__(self, soft_targets, weight = None, gamma = 2.0, reduction = 'mean', device = 'cuda'):
        super(PnLFocalLoss,self).__init__()
        self.weight = weight
        self.gamma = gamma
        self.reduction = reduction
        self.soft_targets = torch.tensor(soft_targets, dtype = torch.float32).to(device)
...
def validate_engine(model, val_loader, criterion, device, confidence_threshold: 0.6):
    ...
    with torch.no_grad(): # stop gradient
        for data, label in val_loader:
            ...
            weak_signals_mask = (predicted != 1) & (max_probs < confidence_threshold)
            predicted[weak_signals_mask] = 1
